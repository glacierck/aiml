...
public class Utils {
	...
	/**
	 * @param precision
	 *            quantas casas decimais para a precisao
	 * @return MathContext
	 */
	 ...
	 * @param fileName
	 * @param isForAppend
	 *            true se for para acrescentar ao ficheiro
	 * @param data
	 *            os dados vem sob a forma de um ArrayList<String[][2]> onde String[i][0] contem os dados e String[i][1] contem a mascara 
	 */
	public static void appendToFile(String fileName, boolean isForAppend,
		 ...
		 * isForAppend)); StringBuffer sb = null; for (String[] lineData : data) {
		 * String[] col = new String[mask]; System.arraycopy(lineData, 0, col,
		 * 0, lineData.length); for(int i = lineData.length; i < mask; i++){
		 * col[i] = " "; } sb = new StringBuffer(); for(String entry : col){
		 * sb.append(entry); } out.write(sb.toString() + "\n"); } out.close();
		 * }catch(Exception ex){ ex.printStackTrace(); }
		 */
		/*
		 * try { if (data.get(0).length != mask.length) throw new
		 * IllegalArgumentException( "mask.length tem de ser igual a
		 * data.length"); BufferedWriter out = new BufferedWriter(new
		 * FileWriter(fileName, isForAppend)); StringBuffer sb = null; boolean
		 * flag = false; for (String[] lineData : data) { sb = new
		 * StringBuffer(); for (int i = 0; i < lineData.length; i++) { int len =
		 * lineData[i].length(); if (!lineData[i].startsWith("-")) { flag =
		 * true; sb.append(" "); } sb.append(lineData[i]); if (flag) { for (int
		 * j = len; j < mask[i] - 1; j++) { sb.append(" "); } flag = false; }
		 * else { for (int j = len; j < mask[i]; j++) { sb.append(" "); } } }
		 * out.write(sb.toString() + "\n"); } out.close(); } catch (IOException
		 * e) { e.printStackTrace(); } catch (IllegalArgumentException ex) {
		 * ex.printStackTrace(); }
		 */
	}
